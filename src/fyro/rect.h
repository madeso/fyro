...
struct Rect
{
	...
	constexpr Rect<T>(T l, T b, T r, T t)
		: left(l)
		, bottom(b)
		, right(r)
		, top(t)
	...
	constexpr Rect<T>(T width, T height)
		: left(0)
		, bottom(0)
		, right(width)
		, top(height)
	...
	constexpr Rect<T> zero() const
	{
		return {0, 0, right - left, top - bottom};
	}

	constexpr Rect<T> scale_uniform(T s) const
	{
		return {left * s, bottom * s, right * s, top * s};
	}

	constexpr bool is_inside_inclusive(T x, T y) const
	{
		return x >= left && x <= right && y >= bottom && y <= top;
	}

	constexpr Rect<T> translate(T dx, T dy) const
	{
		return {left + dx, bottom + dy, right + dx, top + dy};
	}

	template<glm::qualifier Q>
	constexpr Rect<T> translate(const glm::vec<2, T, Q>& v) const
	{
		return translate(v.x, v.y);
	}

	template<glm::qualifier Q>
	constexpr glm::vec<2, T, Q> to01(const glm::vec<2, T, Q>& p) const
	{
		return {(p.x - left) / get_width(), (p.y - bottom) / get_height()};
	}

	template<glm::qualifier Q>
	constexpr glm::vec<2, T, Q> from01(const glm::vec<2, T, Q>& p) const
	{
		return {left + (p.x * get_width()), bottom + (p.y * get_height())};
	}
	...
	constexpr T get_scale_for_height(T new_height) const
	{
		const T old_height = get_height();
		return new_height / old_height;
	}

	constexpr T get_scale_for_width(T new_width) const
	{
		const T old_width = get_width();
		return new_width / old_width;
	}

	constexpr Rect<T> set_height(T new_height) const
	{
		return scale_uniform(get_scale_for_height(new_height));
	}

	static constexpr Rect<T> from_xywh(T x, T y, T w, T h)
	{
		return {x, y, x + w, y + h};
	}

	constexpr Rect<T> set_bottom_left(T x, T y)
	{
...
template<typename T>
Rect<T> cut_left(Rect<T>* rect, T a)
{
...
template<typename T>
Rect<T> cut_right(Rect<T>* rect, T a)
{
...
template<typename T>
Rect<T> cut_top(Rect<T>* rect, T a)
{
...
template<typename T>
Rect<T> cut_bottom(Rect<T>* rect, T a)
{
	T old_top = rect->top;
...
template<typename T>
Rect<T> rectcut_cut(const RectCut<T>& rectcut, T a)
{
	switch (rectcut.side)
	{
	...
	default: DIE("invalid rectcut side"); return cut_bottom(rectcut.rect, a);
	}
}

template<typename T>
bool rect_intersect(const Rect<T>& lhs, const Rect<T>& rhs)
{
	return ! (
		rhs.left >= lhs.right || rhs.right <= lhs.left || rhs.top <= lhs.bottom
		|| rhs.bottom >= lhs.top
	);
}
